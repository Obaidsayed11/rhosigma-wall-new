...
export default function CareerL2page({ job }: { job: CareersProps }) {
  const router = useRouter();
  const url = usePathname();
  const errormessage = useRef<HTMLParagraphElement | null>(null);
  const { executeRecaptcha } = useGoogleReCaptcha();
  ...
  const onSubmit = async (submitdata: ModelType) => {
    ...
    try {
      ...
      const axiosError = error as AxiosError<{ message?: string }>;
      console.error("There was an error submitting:", axiosError);
      ...
      reset();
      setTimeout(() => {
        if (errormessage.current) errormessage.current.innerText = "";
      }, 6000);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 my-10 sm:p-10 text-text-secondary border border-hover-section py-10">
      ...
      <div className="flex flex-wrap justify-between mb-6 text-sm border-b border-hover-section pb-5">
        <p>
      ...
      <div className="bg-gray-50 p-5 rounded-md">
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <textarea
            placeholder="Your message here"
            className="border border-gray-300 px-4 py-3 h-24 resize-none w-full"
          />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between">
            ...
            <button
              type="submit"
              className="mt-4 sm:mt-0 bg-red-500 text-white px-6 py-2 rounded"
            >
              Apply
            </button>
          </div>
