...
export default function AllProductsList({ groupedProducts }: any) {
  ...
  const toggleCategory = (category: string) => {
    setExpandedCategories((prev) => ({
      ...prev,
      [category]: prev[category] ? prev[category] + 3 : 6,
    }));
  };
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-lg text-gray-600">
            Explore our complete range of valve automation solutions
          </p>
        </div> */}
          ...
          {Object.entries(groupedProducts).map(
            ([category, products]: [string, any]) => {
              const visibleCount = expandedCategories[category] || 3;
              ...
              return (
                <div key={category} className="mb-16" id={category}>
                  {/* Category Header */}
                  <div className="mb-8">
  <h2 className="text-3xl font-bold text-text-secondary mb-2">
    <span className="relative inline-block">{category}
